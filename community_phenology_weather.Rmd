...
kphen_pop_fl$productivity_mean <- kphen_pop_fl$productivity_sum/kphen_pop_fl$n
kphen_pop_fr$productivity_mean <- kphen_pop_fr$productivity_sum/kphen_pop_fr$n
```
columns:\
year - year\
month - month\
...
productivity_mean - mean flower or fruit productivity across individuals
...
par(mfrow = c(4,2))
hist(kphen_pop_fl$individuals, xlab ='prop. of individuals', main = 'flower')
hist(kphen_pop_fl$sprich, xlab = 'prop. of taxa', main = '')
hist(kphen_pop_fl$productivity, xlab ='productivity', main = '')
hist(kphen_pop_fl$productivity_mean, xlab ='mean productivity', main = '')
hist(kphen_pop_fr$individuals, xlab ='prop. of individuals', main = 'fruit')
hist(kphen_pop_fr$sprich, xlab = 'prop. of taxa', main = '')
hist(kphen_pop_fr$productivity, xlab ='productivity', main = '')
hist(kphen_pop_fr$productivity_mean, xlab ='mean productivity', main = '')
```

Evaluate the relationship between community phenology and weather 
```{r, echo = T, warning = F, message = T}
# examine the correlation plots between the independent variables 
corrgram(kphen_pop_fl[,c('rain','temp','rad')], lower.panel =panel.pts, upper.panel= panel.conf,diag.panel = panel.density)
```
Temperature and solar radiation are highly correlation, so should not be put into the same model. 

Visualize community phenology across months 
...
plot_grid(fli_box, fri_box, flsp_box, frsp_box, flprod_box, frprod_box, nrow = 4, ncol=2)
...
```{r}
# generate boxplot of prop of individuals fruiting across months
fri_box2  <- ggplot(subfruit,
                   aes(x=as.factor(month),y=fruited)) +
                geom_boxplot(outlier.colour="lightgrey") +
                theme_minimal(base_size=10) +
                theme(legend.position = 'none') +
                labs(x = 'month',y ='proportion of individuals', title = 'fruiting') +
                scale_fill_manual(values = c('grey80','grey40')) + 
                facet_wrap(~dataset, 
                           nrow = 1, 
                           ncol = 2, 
...
# generate boxplot of prop of taxa fruiting across months
frsp_box2 <- ggplot(subfruit,
                   aes(x=as.factor(month),y=sprich))+
                geom_boxplot(outlier.colour="lightgrey")+
                theme_minimal(base_size=10) +
                theme(legend.position = 'none') +
                labs(x = 'month',y ='proportion of taxa') +
                facet_wrap(~dataset, 
                           nrow = 1, 
                           ncol = 2, 
                           strip.position = 'right')

frprod_box2  <- ggplot(subfruit,
                      aes(x=as.factor(month),y=fruit.productivity)) +
                  geom_boxplot(outlier.colour="lightgrey")+
                  theme_minimal(base_size=10) +
                  theme(legend.position = 'none') +
                  labs(x = 'month',y ='fruit productivity') +
                  facet_wrap(~dataset, 
                             nrow = 1, 
                             ncol = 2, 
                             strip.position = 'right')
...
plot_grid(fri_box, frsp_box, frprod_box, nrow = 3, ncol=1)
plot_grid(frsp_box2, frprod_box2, frcomp_box, ravprod_box, nrow = 4, ncol=1)

plot_grid(ravprod_box, frcomp_box_vv, frcomp_box_ss, nrow = 3, ncol=1)
```

Visualize community phenology across years
```{r, echo = T, message = T, warning = F}
...
plot_grid(fli_box, fri_box, flsp_box, frsp_box, flprod_box, frprod_box, nrow = 3, ncol=2)
```

Visualize phenology as time series across dates
```{r, echo =T, message = F, warning=F}
# plot time series of prop flowering individuals 
fl_ploti <- ggplot(kphen_pop_fl, aes(x = Date, y = individuals)) +
              geom_line(size = 1) +
...
fr_ploti <- ggplot(kphen_pop_fr, aes(x = Date, y = individuals)) +
                geom_line(size = 1) +
                geom_line(aes(x = Date, y =rain/1200), 
                          linetype = 'solid', 
                          colour = '#56B4E9',
...
fl_plotsp <- ggplot(kphen_pop_fl, aes(x = Date, y = sprich)) +
                geom_line(size = 1) +
                ...
                geom_line(aes(x = Date, y =temp/60), 
                          ...
                          colour = '#D55E00',
                          linewidth = 0.7) +
                scale_y_continuous(sec.axis = sec_axis(~.*1800, name = 'temperature (C)'), 
                                   name = 'prop. of taxa') +
                geom_rect(aes(xmin = as.Date('2010-10-01'), xmax = as.Date('2010-12-31'),
                    ymin = -Inf, ymax = Inf), alpha = 0.005) +
                geom_rect(aes(xmin = as.Date('2011-10-01'), xmax = as.Date('2011-12-31'),
...
fr_plotsp <- ggplot(kphen_pop_fr, aes(x = Date, y = sprich))+
                geom_line(size = 1) +
                geom_line(aes(x = Date, y =rain/1800), 
                          linetype = 'solid', 
                          colour = '#56B4E9',
...
# plot time series of flowering productivity
fl_plotprod <- ggplot(kphen_pop_fl, aes(x = Date, y = productivity)) +
                  geom_line(size = 1) +                
                  ...
                  geom_line(aes(x = Date, y =temp/300), 
                            ...
                            colour = '#D55E00',
                            linewidth = 0.7) +
                  scale_y_continuous(sec.axis = sec_axis(~.*4000, name = 'solar radiation'), 
                                     name = 'prop. productivity') +
                  geom_rect(aes(xmin = as.Date('2010-10-01'), xmax = as.Date('2010-12-31'),
                        ymin = -Inf, ymax = Inf), alpha = 0.005) +
                  geom_rect(aes(xmin = as.Date('2011-10-01'), xmax = as.Date('2011-12-31'),
                        ymin = -Inf, ymax = Inf), alpha = 0.005) +
...
fr_plotprod <- ggplot(kphen_pop_fr, aes(x = Date, y = productivity)) +
                  geom_line(size = 1) +                
                  geom_line(aes(x = Date, y =rain/4000), 
                            linetype = 'solid', 
                            colour = '#56B4E9',
...
plot_grid(fl_ploti,fl_plotsp,fl_plotprod, fr_ploti,fr_plotsp,fr_plotprod, nrow = 6, ncol=1)

# save plot
ml <- marrangeGrob(list(fl_ploti,fl_plotsp,fl_plotprod, fr_ploti,fr_plotsp,fr_plotprod), nrow = 6, ncol=1)
# save plot
ggsave(file = paste0(figpath, '/comm_phen_timeseries_prop.png'),
       plot = ml,
       width = 6.5, 
       height = 8, 
       units = 'in')
