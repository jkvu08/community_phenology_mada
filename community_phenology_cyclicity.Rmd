...
```{r, echo = T, warning = F, message = F}
# Run Chi Squared statistics to see if the frequency of cycle classes differs from random expectation (p = 0.2)
...
Run post-hoc chisq and Fisher test to see if frequency of dominant cycles was higher than the frequency of all other cycles
```{r, echo = T, warning = F, message = F}
# Compare whether the dominant cycle frequency is sig. higher than the second dominant cycle frequency
# If dominant cycle frequency is sig. higher than the second dominant cycle frequency, then it will also be sig. higher than all other cycles
# function for identifying the dominant category and conducting binomial test
dom_binom <- function(df, forest, st){
  # df: dominant cycle table 
  # forest: specify which forest: 'Sangasanga' or 'Vatovavy'
  # st: specify which stage: 'flower' or 'fruit'
  df <- df %>% filter(dataset == forest & stage == st & scale == 'individual') # filter dataframe by forest and stage
  df <- arrange(df, desc(n))
  dom_cycle <- df$cycle_category[1] # get the dominant cycle category
  dominant_n <- df$n[1] # get the number of times that dominant cycle category occurred
  second_n <- df$n[2] # get the number of times that dominant cycle category occurred
  ...
  binom_out <- binom.test(x=c(dominant_n,second_n),
                          alternative='greater')
  pval <- binom_out$p.value # extract significance value
  return(c(dom_cycle, total_n, dominant_n, second_n, pval))
}
...
for (i in c('Sangasanga','Vatovavy')) { # for each forest community 
  for (j in c('flower','fruit')) { # for each phenology stage
      binom_output <- dom_binom(dompop_summary,i,j) # run binomial test
      binom_results <- rbind(binom_results, c(i,j,binom_output)) # append results to binomial results test table
  }
}

# assign column labels of binom results 
colnames(binom_results) <- c('community','stage','dom_cycle','total_cycles_n','dom_cycle_n','second_cycle_n','sig') # assign column names
...
dur_plot <- ggplot(cycle_summary  %>% filter(dataset == 'sssw'), aes(x = duration_median)) +
...
int_plot <- ggplot(cycle_summary %>% filter(dataset == 'sssw'), aes(x = interval_median)) +
                  geom_histogram(color = 'black', fill = 'darkgray') +
                  geom_vline(data=community_sum %>% filter(dataset == 'sssw'), 
                  ...
                  facet_nested_wrap(~scale + stage,
                                    ...
                                    scales = 'free')
# arrange figure 
cycle_medians <- grid.arrange(length_plot, dur_plot, int_plot, 
                             ncol = 3, 
                             nrow = 1)
...
# arrange figure 
cycle_medians <- grid.arrange(length_plot, dur_plot, int_plot, ncol = 3, nrow = 1)
...
# arrange figure 
cycle_sds <- grid.arrange(length_plot, dur_plot, int_plot, ncol = 3, nrow = 1)
...
```{r, echo=T, warning=F, message=F}
# get difference between the cycle characteristics between the two communities 
# evens indices are Vatovavy, odd indices are Sangasanga
community_diff <- community_sum[c(2,4,6,8),5:ncol(community_sum)] - community_sum[c(1,3,5,7),5:ncol(community_sum)]
community_diff$scale <- c('individual','individual','species','species') # reassign scale 
community_diff$stage <- c('flower','fruit','flower','fruit') # reassign phenological stage

# permutation function evaluate whether cycle characteristics between communities sig. deviates from 0 
perm_cycle_char <- function(df,sc,st){
  ...
  # recalculate the the cycle characteristics after permutation 
  submetrics <- subdf %>% group_by(dataset) %>% 
                          summarise(length_median = median(length_median,na.rm=T), 
                                    length_mad = mad(length_median,na.rm=T), 
                                    duration_median = median(duration_median,na.rm=T),        
                                    duration_mad = mad(duration_median,na.rm=T),
                                    interval_median = median(interval_median,na.rm=T),
...
perm_pvalue <- function(obs_df, perm_df,sc, st, metric){
  ...
  # perm_df = permutation dataframe
  # sc = individual or species
  # st = flower or fruit
  # metric = cycle characteristic being evaluated
  subdf <- obs_df %>% filter(stage == st & scale == sc) # subset to appropriate scale and stage
  metname <- colnames(obs_df)[metric] # name of the metric (cycle characteristic)
  ref <- subdf[1,metric] # pull the observed test statistic
  # find the p-value
  perm_metric <- perm_df[,metname] # extract permuted test statistic
  pval <- sum(abs(perm_metric) >= abs(ref))/length(perm_metric) # get proportion of values that are more extreme than the absolute value of the observed test statistic, since a two tailed test is performed
  pval_df <- data.frame(scale = sc, stage = st,metric = metname, obs_value = ref, pvalue = pval) # output info
  return(pval_df)
}

# calculate for flower individual in parallel
...
# calcualate the p-value
fli_pvalues <- foreach(i = 1:6, .combine = 'rbind', .packages = 'dplyr') %dopar%{
...
# calculate p-values
fri_pvalues <- foreach(i = 1:6, .combine = 'rbind', .packages = 'dplyr') %dopar%{
...
# calculate p-values
flsp_pvalues <- foreach(i = 1:6, .combine = 'rbind', .packages = 'dplyr') %dopar%{
...
# calculate p-values
frsp_pvalues <- foreach(i = 1:6, .combine = 'rbind', .packages = 'dplyr') %dopar%{
  pmetrics <- perm_pvalue(community_diff, frsp_cycle_comp, 'species','fruit',i)
  return(pmetrics)
}

# merge p-value tables
...
# print out observation metrics and p-values
comm_char_pvalues
```
