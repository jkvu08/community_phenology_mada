...
setwd("C:/Users/Jannet/Documents/Dissertation/codes/community_phen_mada")
knitr::opts_knit$set(root.dir = "C:/Users/Jannet/Documents/Dissertation/codes/community_phen_mada") 
...
indi_plot <- ggplot(dompop_summary %>% filter(scale == 'individual'),  # subset out individual data
                  aes(x = dataset, y = est, fill = cycle_category)) + # group bar plots by community 
                                                                      # plot the dominant cycle proportions
                                                                      # color by the cycle class
                  geom_bar(stat = 'identity', position = 'dodge', color = 'black') + # side by side bar plots
...
sp_plot <- ggplot(dompop_summary %>% filter(scale == 'species'), # subset out species data 
                  aes(x = dataset, y = est, fill = cycle_category)) + # group barplots by community 
                                                                      # plot the dominant cycle proportions
                                                                      # color by the cycle class
            geom_bar(stat = 'identity', position = 'dodge', color = 'black') + # side by side barplots
            geom_errorbar(aes(ymin = lwr.ci, ymax = upr.ci), # with confidence intervals
                          position = position_dodge(width = 0.9), colour = 'black', width = 0.2) +
            theme_bw() + # set plot theme
...
dom_plot <- ggplot(dompop_summary,  # subset out individual data
                  aes(x = dataset, y = est, fill = cycle_category)) + # group barplots by community 
                                                                      # plot the dominant cycle proportions
                                                                      # color by the dominant cycle class
                  geom_col(color = 'black',position = 'fill', size = 0.2) + # create stacked bar plot 
                  geom_text(data = dompop_n, aes(x = dataset, y = 1.05, label = total_n), size = 2) +
                  theme_bw() + # set plot theme
                  # further modify theme aesthetics 
